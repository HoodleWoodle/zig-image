...
const RGBA32 = Image.RGBA32;

pub const TestImageColorization = enum { Default, Alpha, Mono };

const red: RGBA32 = .{ .r = 0xF7, .g = 0x08, .b = 0x08 };
const green: RGBA32 = .{ .r = 0x08, .g = 0xEF, .b = 0x08 };
const blue: RGBA32 = .{ .r = 0x08, .g = 0x08, .b = 0xE7 };
const gray: RGBA32 = .{ .r = 0x5A, .g = 0x5A, .b = 0x5A };
const black: RGBA32 = .{};
const white: RGBA32 = .{ .r = 0xFF, .g = 0xFF, .b = 0xFF };
...
pub fn testReadImage8x4Success(relative_path: []const u8, comptime col: TestImageColorization) !void {
    const colors = switch (col) {
        ...
        .Alpha => .{ red, green, blue, gray, .{ .a = 0x00 } },
...
pub fn testReadImageFailure(relative_path: []const u8, expected_error: Image.Error) !void {
    var image = readTestImage(relative_path);
    try std.testing.expectError(expected_error, image);
}

test "reading: unknown format" {
    try testReadImageFailure("images/test.noimg", Image.Error.FormatUnkown);
}
